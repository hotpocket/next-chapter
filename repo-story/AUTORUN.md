...
- **Model split — Fable writes, Opus researches.** The main session is pinned to Fable via `.claude/settings.json` (loud failure if unavailable — do not fall back silently) and does the planning phases: 1 survey, 3 inventory, 5a beats, chapter ordering. Phases 5b/5c run via the `narrator` agent, also pinned to Fable — narration is judged as prose. Phases 2 and 4 run via `explorer`, `code-researcher` and `prompt-researcher`, each pinned to Opus. Never spawn generic subagents for those phases and never write sections inline. See [docs/adr/0002](docs/adr/0002-fable-writes-opus-researches.md).
